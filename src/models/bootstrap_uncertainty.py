...
single point-estimate probability, refit the model 10,000 times on
resampled historical training data and report the spread. This exists
because a single number (e.g. "73%") hides how much that number would
have wobbled if the historical dataset had come out even slightly
differently -- with only ~142 WC knockout matches to train on, that
wobble is real and worth showing, not just the point estimate.
...
uncertainty given a small sample.
"""
import sys
...
def bootstrap_win_probability(feature_rows, query_features, n_boot: int = 10000, seed: int = 42):
    ...
    for i in range(n_boot):
        ...
        if len(np.unique(y_boot)) < 2:
            # Degenerate resample (all one class) -- rare at n=142+ but not
            # impossible; reuse the previous valid sample rather than crash
...
if __name__ == "__main__":
    ...
    for label, query in [
        ...
        t0 = time.time()
        point, samples = bootstrap_win_probability(feature_rows, query, n_boot=10000)
        stats = summarize_bootstrap(point, samples)
        elapsed = time.time() - t0
        print(f"{label}:")
        print(f"  Point estimate: {point:.3f}")
        print(f"  90% CI (10,000 bootstrap refits): [{stats['ci_low']:.3f}, {stats['ci_high']:.3f}] (std={stats['std']:.3f})")
